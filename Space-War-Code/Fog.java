...
public class Fog extends Actor {
    ...
    public Fog(int seed, int animationMilliseconds) {
    ...
    private void animation() {
        if (animationMilliseconds % speed == 0) {
            moveOnce();
        }
    }
    ...
    public void act() {
        ...
        if (getX() > getWorld().getWidth() + (getImage().getWidth() / 2) || (getX() < -(getImage().getWidth()) && getY() > getWorld().getHeight() / 2) || getY() > getWorld().getHeight() + (getImage().getHeight() / 2)) {
            ...
            if (seed % 2 == 0) {
                getWorld().addObject(new Fog(seed, animationMilliseconds), getWorld().getWidth() - 200, -500);
            } else {
                getWorld().addObject(new Fog(seed, animationMilliseconds), 200, -500);
            }
